...
import { validateSDLSync, validateSync } from '../validate.ts';
...
export function expectValidationErrorsWithSchema(
  ...
  const errors = validateSync(schema, doc, [rule]);
  return expect(JSON.parse(JSON.stringify(errors)));
}
...
export function expectSDLValidationErrors(
  ...
  const errors = validateSDLSync(doc, schema, [rule]);
  return expect(JSON.parse(JSON.stringify(errors)));
}
