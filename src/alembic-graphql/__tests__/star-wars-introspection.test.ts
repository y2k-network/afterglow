...
function queryStarWars(source: string) {
  const result = Effect.runSync(graphql({ schema: StarWarsSchema, source }));
...
describe("Star Wars Introspection Tests", () => {
  ...
  it("Allows querying the schema for types", () => {
    const data = queryStarWars(`{ __schema { types { name } } }`);
  ...
  it("Allows querying the schema for query type", () => {
    expect(queryStarWars(`{ __schema { queryType { name } } }`)).toEqual({
      __schema: { queryType: { name: "Query" } },
    });
  });

  it("Allows querying the schema for a specific type", () => {
    expect(queryStarWars(`{ __type(name: "Droid") { name } }`)).toEqual({
      __type: { name: "Droid" },
    });
  });

  it("Allows querying the schema for an object kind", () => {
    expect(queryStarWars(`{ __type(name: "Droid") { name kind } }`)).toEqual({
      __type: { name: "Droid", kind: "OBJECT" },
    });
  });

  it("Allows querying the schema for an interface kind", () => {
    expect(queryStarWars(`{ __type(name: "Character") { name kind } }`)).toEqual({
      __type: { name: "Character", kind: "INTERFACE" },
    });
  });

  it("Allows querying the schema for object fields", () => {
    const data = queryStarWars(`{ __type(name: "Droid") { name fields { name type { name kind } } } }`);
  ...
  it("Allows querying the schema for nested object fields", () => {
    const data = queryStarWars(`{ __type(name: "Droid") { name fields { name type { name kind ofType { name kind } } } } }`);
    ...
    expect(data).toEqual({
      __type: {
        ...
        fields: [
          ...
          { name: "friends", type: { name: null, kind: "LIST", ofType: { name: "Character", kind: "INTERFACE" } } },
          { name: "appearsIn", type: { name: null, kind: "LIST", ofType: { name: "Episode", kind: "ENUM" } } },
          { name: "secretBackstory", type: { name: "String", kind: "SCALAR", ofType: null } },
          { name: "primaryFunction", type: { name: "String", kind: "SCALAR", ofType: null } },
        ],
      },
    });
  });
  ...
  it("Allows querying the schema for field args", () => {
    const data = queryStarWars(`
      { __schema { queryType { fields { name args { name description type { name kind ofType { name kind } } defaultValue } } } } }
    `);
  ...
  it("Allows querying the schema for documentation", () => {
    expect(queryStarWars(`{ __type(name: "Droid") { name description } }`)).toEqual({
      __type: {
        name: "Droid",
