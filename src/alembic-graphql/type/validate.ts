...
import { isEqualType, isTypeSubTypeOf } from '../utilities/type-comparators.ts';
...
/**
 * Effect variant of validateSchema. Fails with the array of validation
 * errors when the schema is invalid; otherwise succeeds with void.
 */
export function validateSchemaEffect(
  schema: GraphQLSchema,
): Effect.Effect<void, ReadonlyArray<GraphQLError>, never> {
  const errors = validateSchema(schema);
  if (errors.length > 0) return Effect.fail(errors);
  return Effect.void;
}
...
class SchemaValidationContext {
  ...
  reportError(
    ...
    this._errors.push(new GraphQLSchemaValidationError(message, { nodes: _nodes }));
  }
