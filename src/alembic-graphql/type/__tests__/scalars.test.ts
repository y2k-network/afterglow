...
import { parseValue as parseValueToAST } from '../../language/parser.ts';
...
describe('Type System: Specified scalar types', () => {
  describe('GraphQLInt', () => {
    ...
    it('serialize', () => {
      ...
      expect(serialize(-1)).toBe(-1);
      expect(serialize(1e5)).toBe(100000);
      ...
      expect(serialize(customValueOfObj)).toBe(5);
  ...
  describe('GraphQLFloat', () => {
    it('parseValue', () => {
      ...
      expect(parseValue(Math.PI)).toBe(Math.PI);
    ...
    it('parseLiteral', () => {
      ...
      expect(parseLiteral('0')).toBe(0);
      expect(parseLiteral('-1')).toBe(-1);
      expect(parseLiteral('0.1')).toBe(0.1);
      expect(parseLiteral(Math.PI.toString())).toBe(Math.PI);
    ...
    it('serialize', () => {
      ...
      expect(serialize(1)).toBe(1.0);
      expect(serialize(0)).toBe(0.0);
      expect(serialize('123.5')).toBe(123.5);
      expect(serialize(-1)).toBe(-1.0);
      expect(serialize(0.1)).toBe(0.1);
      expect(serialize(1.1)).toBe(1.1);
      expect(serialize(-1.1)).toBe(-1.1);
      expect(serialize('-1.1')).toBe(-1.1);
      expect(serialize(false)).toBe(0.0);
      expect(serialize(true)).toBe(1.0);
      ...
      expect(serialize(customValueOfObj)).toBe(5.5);
  ...
  describe('GraphQLString', () => {
    ...
    it('parseLiteral', () => {
      ...
      expect(parseLiteral('"""bar"""')).toBe('bar');
    ...
    it('serialize', () => {
      ...
      expect(serialize(valueOfAndToJSONValue)).toBe('valueOf string');

      const onlyToJSONValue = { toJSON };
      expect(serialize(onlyToJSONValue)).toBe('toJSON string');
  ...
  describe('GraphQLID', () => {
    it('parseValue', () => {
      ...
      expect(parseValue(9007199254740991)).toBe('9007199254740991');
      expect(parseValue(-9007199254740991)).toBe('-9007199254740991');
