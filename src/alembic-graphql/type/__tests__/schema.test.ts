...
import { printSchema } from '../../utilities/print-schema.ts';
...
describe('Type System: Schema', () => {
  it('Define sample schema', () => {
    ...
    expect(printSchema(schema)).toBe(dedent`
      """Sample schema"""
      schema {
        query: Query
        mutation: Mutation
  ...
  describe('Root types', () => {
    ...
    it('defines a query root', () => {
      ...
      expect(schema.getQueryType()).toBe(testType);
      expect(schema.getTypeMap()).toHaveProperty('TestType');
    });

    it('defines a mutation root', () => {
      const schema = new GraphQLSchema({ mutation: testType });
      expect(schema.getMutationType()).toBe(testType);
      expect(schema.getTypeMap()).toHaveProperty('TestType');
    });

    it('defines a subscription root', () => {
      const schema = new GraphQLSchema({ subscription: testType });
      expect(schema.getSubscriptionType()).toBe(testType);
      expect(schema.getTypeMap()).toHaveProperty('TestType');
    });
  });
  ...
  describe('Type Map', () => {
    it('includes interface possible types in the type map', () => {
      ...
      expect(schema.getType('SomeInterface')).toBe(SomeInterface);
      expect(schema.getType('SomeSubtype')).toBe(SomeSubtype);

      expect(schema.isSubType(SomeInterface, SomeSubtype)).toBe(true);
    });
    ...
    it("includes interface's thunk subtypes in the type map", () => {
      ...
      expect(schema.getType('SomeInterface')).toBe(SomeInterface);
      expect(schema.getType('AnotherInterface')).toBe(AnotherInterface);
      expect(schema.getType('SomeSubtype')).toBe(SomeSubtype);
    });
    ...
    it('includes nested input objects in the map', () => {
      ...
      expect(schema.getType('SomeInputObject')).toBe(SomeInputObject);
      expect(schema.getType('NestedInputObject')).toBe(NestedInputObject);
    });
    ...
    it('includes input types only used in directives', () => {
      ...
      expect(schema.getTypeMap()).toHaveProperty('Foo');
      expect(schema.getTypeMap()).toHaveProperty('Bar');
    });
  });
  ...
  it('can be Object.toStringified', () => {
    ...
    expect(Object.prototype.toString.call(schema)).toBe(
