...
describe('Lexer', () => {
  ...
  it('can be Object.toStringified, JSON.stringified, or jsutils.inspected', () => {
    ...
    expect(Object.prototype.toString.call(lexer)).toBe('[object Lexer]');

    expect(Object.prototype.toString.call(token)).toBe('[object Token]');
    expect(JSON.stringify(token)).toBe(
      '{"kind":"Name","value":"foo","line":1,"column":1}',
    );
    expect(inspect(token)).toBe(
      '{ kind: "Name", value: "foo", line: 1, column: 1 }',
    );
  });
...
`),
  ...
  it('produces double linked list of tokens, including comments', () => {
    ...
    do {
      ...
      expect(endToken.kind).not.toBe(TokenKind.COMMENT);
    } while (endToken.kind !== TokenKind.EOF);

    expect(startToken.prev).toBe(null);
    expect(endToken.next).toBe(null);

    const tokens = [];
    for (let tok: Token | null = startToken; tok; tok = tok.next) {
      if (tokens.length) {
        // Tokens are double-linked, prev should point to last seen token.
        expect(tok.prev).toBe(tokens[tokens.length - 1]);
      }
      tokens.push(tok);
    }
...
describe('isPunctuatorTokenKind', () => {
  ...
  it('returns true for punctuator tokens', () => {
    expect(isPunctuatorToken('!')).toBe(true);
    expect(isPunctuatorToken('$')).toBe(true);
    expect(isPunctuatorToken('&')).toBe(true);
    expect(isPunctuatorToken('(')).toBe(true);
    expect(isPunctuatorToken(')')).toBe(true);
  ...
  it('returns false for non-punctuator tokens', () => {
    expect(isPunctuatorToken('')).toBe(false);
    expect(isPunctuatorToken('name')).toBe(false);
    expect(isPunctuatorToken('1')).toBe(false);
    expect(isPunctuatorToken('3.14')).toBe(false);
    expect(isPunctuatorToken('"str"')).toBe(false);
    expect(isPunctuatorToken('"""str"""')).toBe(false);
  });
});
