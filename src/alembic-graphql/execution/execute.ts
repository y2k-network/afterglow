...
export function execute<R = never>(
  ...
  if (compiled !== null) {
    ...
    return args.options?.trace === true
      ? effect.pipe(Effect.withSpan('alembic.execute'))
      : effect;
  }
...
const GENERATED_EXECUTOR_HOT_THRESHOLD = readNonNegativeIntEnv('ALEMBIC_GRAPHQL_CODEGEN_HOT_THRESHOLD', 32);
const GENERATED_EXECUTOR_HOT_WINDOW_MS = readNonNegativeIntEnv('ALEMBIC_GRAPHQL_CODEGEN_HOT_WINDOW_MS', 60_000);
const GENERATED_EXECUTOR_MAX_ARTIFACTS = readNonNegativeIntEnv('ALEMBIC_GRAPHQL_CODEGEN_MAX_ARTIFACTS', 128);
const GENERATED_EXECUTOR_MIN_COMPLEXITY = readNonNegativeIntEnv('ALEMBIC_GRAPHQL_CODEGEN_MIN_COMPLEXITY', 1);
const GENERATED_EXECUTOR_RECOMPILE_COOLDOWN_HITS = readNonNegativeIntEnv('ALEMBIC_GRAPHQL_CODEGEN_RECOMPILE_COOLDOWN_HITS', 1024);
const GENERATED_NODE_WRITER_HOT_THRESHOLD = readNonNegativeIntEnv('ALEMBIC_GRAPHQL_NODE_CODEGEN_HOT_THRESHOLD', 32);
const GENERATED_NODE_WRITER_MIN_COMPLEXITY = readNonNegativeIntEnv('ALEMBIC_GRAPHQL_NODE_CODEGEN_MIN_COMPLEXITY', 3);
const GENERATED_NODE_WRITER_MAX_WRITERS = readNonNegativeIntEnv('ALEMBIC_GRAPHQL_NODE_CODEGEN_MAX_WRITERS', 512);
...
class GraphExecutionArtifact {
  ...
  private makeGeneratedExecutor(): GeneratedArtifactExecutor | undefined {
    ...
    const source = `
      "use strict";
      const GRAPH_UNSUPPORTED_VALUE = Symbol.for("alembic.graph.unsupported");
      ${writers}
...
function compileSelection(
  ...
  for (const selection of selections) {
    ...
    const projection = fieldDef.extensions.alembicProjection;
    const relayGlobalId = fieldDef.extensions.alembicRelayGlobalId;
    const resolverPlan = fieldDef.extensions.alembicResolver;
