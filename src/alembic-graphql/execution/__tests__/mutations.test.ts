...
class Root {
  ...
  promiseToChangeTheNumber(newNumber: number): Effect.Effect<NumberHolder> {
    return Effect.sync(() => this.immediatelyChangeTheNumber(newNumber));
  }
  ...
  promiseAndFailToChangeTheNumber(): Effect.Effect<NumberHolder, Error> {
    return Effect.fail(new Error('Cannot change the number'));
  }
}
