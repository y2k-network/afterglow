...
const throwingData = {
  ...
  promise() {
    return Effect.fail(promiseError);
  },
  promiseNonNull() {
    return Effect.fail(promiseNonNullError);
  },
  syncNest() {
    return throwingData;
  },
  ...
  promiseNest() {
    return Effect.succeed(throwingData);
  },
  promiseNonNullNest() {
    return Effect.succeed(throwingData);
  },
...
const nullingData = {
  ...
  promiseNest() {
    return Effect.succeed(nullingData);
  },
  promiseNonNullNest() {
    return Effect.succeed(nullingData);
  },
...
function sortErrors<T extends { errors?: ReadonlyArray<any> }>(result: T): T {
...
describe('Execute: handles non-nullable types', () => {
  ...
  describe('nulls a complex tree of nullable fields, each', () => {
    ...
    it('that throws', async () => {
      ...
      expectJSON(sortErrors(result)).toDeepEqual(sortErrors({
        data,
        errors: [
          {
            message: syncError.message,
