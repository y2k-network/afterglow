...
export function extendSchema(
  ...
  return extendSchemaSync(schema, documentAST, options);
}

export function extendSchemaEffect(
  schema: GraphQLSchema,
  documentAST: DocumentNode,
  options?: Options,
): Effect.Effect<GraphQLSchema, GraphQLError, never> {
  return Effect.try({
    try: () => extendSchemaSync(schema, documentAST, options),
    catch: (e) =>
      isGraphQLError(e)
        ? e
...
export function extendSchemaSync(
  schema: GraphQLSchema,
  documentAST: DocumentNode,
  options?: Options,
): GraphQLSchema {
