...
import {
  connectionEdge as connectionEdgeV1,
  deletedId as deletedIdV1,
} from "../mutation-shapes.ts";
import { decodeGlobalId } from "../relay.ts";
import type {
...
export const deletedId = (typename: string, id: string): string =>
  deletedIdV1(typename, id);

export const edgePayload = <T>(
  cursor: string,
  node: T,
): { cursor: string; node: T } => connectionEdgeV1(cursor, node);
