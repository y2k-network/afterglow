...
import { toHttpApp as v1ToHttpApp, type ToHttpAppOptions as V1ToHttpAppOptions } from "../http.ts";
...
export interface ToHttpAppOptions<R, RA extends R> {
  ...
  readonly graphiql?: V1ToHttpAppOptions<unknown>["graphiql"];
  readonly persistedQueries?: V1ToHttpAppOptions<unknown>["persistedQueries"];
  readonly executor?: V1ToHttpAppOptions<unknown>["executor"];
}
...
> => {
  ...
  const v1Options: V1ToHttpAppOptions<Exclude<R, RA>> = {
  ...
  return v1ToHttpApp(schema as any, v1Options) as unknown as Effect.Effect<
    HttpServerResponse.HttpServerResponse,
    never,
    HttpServerRequest.HttpServerRequest
  >;
