...
test("end-to-end: schema with errors aggregates and throws at buildSchema time", () => {
  ...
  expect(() => buildSchema(errorLayer, null)).toThrow(/RELAY-001/);
  try {
    buildSchema(errorLayer, null);
  } catch (e) {
...
test("end-to-end: warnings printed and respect muteLintWarnings", () => {
  ...
  warnings = [];
  buildSchema(layer, null, { muteLintWarnings: ["RELAY-101"] });
