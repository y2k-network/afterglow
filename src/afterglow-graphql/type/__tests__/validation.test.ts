...
import { extendSchema } from '../../utilities/extend-schema.ts';
...
import { assertValidSchema, validateSchema as rawValidateSchema } from '../validate.ts';

function validateSchema(schema: GraphQLSchema) {
  return JSON.parse(JSON.stringify(rawValidateSchema(schema)));
}
...
describe('Type System: Objects must have fields', () => {
  ...
  it('rejects an Object type with missing fields', () => {
    ...
    expect(validateSchema(manualSchema)).toEqual([
      {
        message: 'Type IncompleteObject must define one or more fields.',
      },
    ]);
    ...
    expect(validateSchema(manualSchema2)).toEqual([
      {
        message: 'Type IncompleteObject must define one or more fields.',
      },
    ]);
...
describe('Type System: Union types must be valid', () => {
  ...
  it('rejects a Union type with non-Object members types', () => {
    ...
    for (const memberType of badUnionMemberTypes) {
      ...
      expect(validateSchema(badSchema)).toEqual([
        {
          message:
