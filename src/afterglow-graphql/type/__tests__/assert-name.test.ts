...
describe('assertName', () => {
  it('passthrough valid name', () => {
    expect(assertName('_ValidName123')).toBe('_ValidName123');
  });

  it('throws for non-strings', () => {
    // @ts-expect-error
    expect(() => assertName({})).toThrow('Expected name to be a string.');
  });
...
describe('assertEnumValueName', () => {
  it('passthrough valid name', () => {
    expect(assertEnumValueName('_ValidName123')).toBe('_ValidName123');
  });
  ...
  it('throws for restricted names', () => {
    expect(() => assertEnumValueName('true')).toThrow(
      'Enum values cannot be named: true',
    );
    expect(() => assertEnumValueName('false')).toThrow(
      'Enum values cannot be named: false',
    );
    expect(() => assertEnumValueName('null')).toThrow(
