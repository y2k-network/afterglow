...
import { introspectionFromSchema } from "../../utilities/introspection-from-schema.ts";
...
describe('Type System: Enum Values', () => {
  ...
  it('presents a getValue() API for complex enums', () => {
    ...
    expect(badUsage).toBe(undefined);
  });
  ...
  it('can be introspected without error', () => {
    expect(() => Effect.runSync(introspectionFromSchema(schema))).not.toThrow();
  });
});
