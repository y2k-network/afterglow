...
import { instanceOf } from '../jsutils/instance-of.ts';
import { isObjectLike } from '../jsutils/is-object-like.ts';
import { keyMap } from '../jsutils/key-map.ts';
import { keyValMap } from '../jsutils/key-val-map.ts';
...
import { toObjMap } from '../jsutils/to-obj-map.ts';

import { GraphQLScalarCoercionError } from '../error/graph-ql-error.ts';
...
export type GraphQLResolverResult<T, R = never> =
  | T
  | Effect.Effect<T, unknown, R>;

export type GraphQLSubscribeResult<T, R = never> =
  | Stream.Stream<T, unknown, R>
  | Effect.Effect<Stream.Stream<T, unknown, R>, unknown, R>;

export type GraphQLTypeResolver<TSource, TContext, R = never> = (
  value: TSource,
  context: TContext,
...
export type GraphQLIsTypeOfFn<TSource, TContext, R = never> = (
  source: TSource,
  context: TContext,
  info: GraphQLResolveInfo,
...
> = (
  ...
  context: TContext,
  info: GraphQLResolveInfo,
) => GraphQLResolverResult<TResult, R>;

export type GraphQLSubscribeResolver<
  TSource,
  TContext,
  TArgs = any,
  TResult = unknown,
...
> = (
  ...
  context: TContext,
  info: GraphQLResolveInfo,
) => GraphQLSubscribeResult<TResult, R>;
...
export interface GraphQLFieldExtensions<_TSource, _TContext, _TArgs = any> {
  readonly afterglowProjection?: { readonly _tag: 'Property'; readonly key: string };
  readonly afterglowRelayGlobalId?: { readonly typename: string; readonly key: string };
  readonly afterglowResolver?: {
...
export interface GraphQLFieldConfig<TSource, TContext, TArgs = any> {
  ...
  subscribe?: GraphQLSubscribeResolver<TSource, TContext, TArgs>;
...
export class GraphQLEnumType /* <T> */ {
  ...
  serialize(outputValue: unknown /* T */): Maybe<string> {
    ...
    if (enumValue === undefined) {
      throw new GraphQLScalarCoercionError(
