...
import type { GraphQLError } from '../error/graph-ql-error.ts';
import { syntaxError } from '../error/syntax-error.ts';
...
import { DirectiveLocation } from './directive-location.ts';
 ...
 * Fails with GraphQLError if a syntax error is encountered.
 */
...
export function parseEffect(
  source: string | Source,
  options?: ParseOptions | undefined,
): Effect.Effect<DocumentNode, GraphQLError, never> {
  return Effect.try({
    try: () => parse(source, options),
    catch: (err) => err as GraphQLError,
  }).pipe(Effect.withSpan('afterglow.parse'));
}
...
export function parseValueEffect(
  source: string | Source,
  options?: ParseOptions | undefined,
): Effect.Effect<ValueNode, GraphQLError, never> {
  return Effect.try({
    try: () => parseValue(source, options),
    catch: (err) => err as GraphQLError,
  });
}
...
export function parseConstValueEffect(
  source: string | Source,
  options?: ParseOptions | undefined,
): Effect.Effect<ConstValueNode, GraphQLError, never> {
  return Effect.try({
    try: () => parseConstValue(source, options),
    catch: (err) => err as GraphQLError,
  });
}
...
export function parseTypeEffect(
  source: string | Source,
  options?: ParseOptions | undefined,
): Effect.Effect<TypeNode, GraphQLError, never> {
  return Effect.try({
    try: () => parseType(source, options),
    catch: (err) => err as GraphQLError,
  });
}
...
export function parseSchemaCoordinateEffect(
  source: string | Source,
): Effect.Effect<SchemaCoordinateNode, GraphQLError, never> {
  return Effect.try({
    try: () => parseSchemaCoordinate(source),
    catch: (err) => err as GraphQLError,
  });
}

/**
 * Synchronous convenience wrappers for migration and tests. These run the
 * Effect immediately and re-throw on syntax errors, preserving the original
 * throw-based ergonomics.
 */
...
export function parseValueSync(
  source: string | Source,
  options?: ParseOptions | undefined,
): ValueNode {
  return parseValue(source, options);
}

export function parseConstValueSync(
  source: string | Source,
  options?: ParseOptions | undefined,
): ConstValueNode {
  return parseConstValue(source, options);
}

export function parseTypeSync(
  source: string | Source,
  options?: ParseOptions | undefined,
): TypeNode {
  return parseType(source, options);
}

export function parseSchemaCoordinateSync(
  source: string | Source,
): SchemaCoordinateNode {
  return parseSchemaCoordinate(source);
}
