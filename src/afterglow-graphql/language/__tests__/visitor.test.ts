...
function checkVisitorFnArgs(ast: any, args: any, isEdited: boolean = false) {
  ...
  expect(parent[key]).not.toBe(undefined);

  expect(path).toBeInstanceOf(Array);
  expect(path[path.length - 1]).toBe(key);

  expect(ancestors).toBeInstanceOf(Array);
  expect(ancestors.length).toBe(path.length - 1);

  if (!isEdited) {
    let currentNode = ast;
    for (let i = 0; i < ancestors.length; ++i) {
      expect(ancestors[i]).toBe(currentNode);

      currentNode = currentNode[path[i]];
      expect(currentNode).not.toBe(undefined);
    }

    expect(parent).toBe(currentNode);
    expect(parent[key]).toBe(node);
  }
}
...
describe('Visitor', () => {
  ...
  it('visits edited node', () => {
    ...
    expect(didVisitAddedField).toBe(true);
  });
