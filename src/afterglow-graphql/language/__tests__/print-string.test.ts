...
import { printString } from '../print-string.ts';

describe('printString', () => {
  it('prints a simple string', () => {
    expect(printString('hello world')).toBe('"hello world"');
  });
  ...
  it('does not escape single quote', () => {
    expect(printString("who's test")).toBe('"who\'s test"');
  });

  it('escapes backslashes', () => {
    expect(printString('escape: \\')).toBe('"escape: \\\\"');
  });
  ...
  it('does not escape supplementary character', () => {
    expect(printString('\u{1f600}')).toBe('"\u{1f600}"');
  });
