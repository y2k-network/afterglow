...
import { getOperationRootType } from '../get-operation-root-type.ts';
...
describe('Deprecated - getOperationRootType', () => {
  it('Gets a Query type for an unnamed OperationDefinitionNode', () => {
    ...
    expect(getOperationRootType(testSchema, operationNode)).toBe(queryType);
  });
  ...
  it('Gets a type for OperationTypeDefinitionNodes', () => {
    ...
    expect(getOperationRootType(testSchema, queryNode)).toBe(queryType);
    expect(getOperationRootType(testSchema, mutationNode)).toBe(
      mutationType,
    );
    expect(getOperationRootType(testSchema, subscriptionNode)).toBe(
      subscriptionType,
    );
  });
  ...
  it('Gets a Mutation type for an OperationDefinitionNode', () => {
    ...
    const operationNode = getOperationNode(doc);
    expect(getOperationRootType(testSchema, operationNode)).toBe(
      mutationType,
    );
  });
