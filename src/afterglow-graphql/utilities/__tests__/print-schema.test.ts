...
import { printIntrospectionSchema, printSchema } from '../print-schema.ts';

function expectPrintedSchema(schema: GraphQLSchema) {
  const schemaText = printSchema(schema);
  // keep printSchema and buildSchema in sync
  expect(printSchema(buildSchema(schemaText))).toBe(schemaText);
...
describe('Type System Printer', () => {
  it('Prints String Field', () => {
    ...
    expectPrintedSchema(schema).toBe(dedent`
      type Query {
        singleField: String
      }
    `);
  ...
  it('Prints String Field With String Arg With Default', () => {
    ...
    expectPrintedSchema(schema).toBe(
