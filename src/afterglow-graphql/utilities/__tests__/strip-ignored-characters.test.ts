...
import { kitchenSinkSDL } from '../../__testUtils__/kitchen-sink-sdl.ts';
...
describe('stripIgnoredCharacters', () => {
  it('strips ignored characters from GraphQL query document', () => {
    ...
    expect(stripIgnoredCharacters(query)).toBe(
      'query SomeQuery($foo:String!$bar:String){someField(foo:$foo bar:$bar){a b{c d}}}',
    );
  });

  it('accepts Source object', () => {
    expect(stripIgnoredCharacters(new Source('{ a }'))).toBe('{a}');
  });
  ...
  it('strips ignored characters from GraphQL SDL document', () => {
    ...
    expect(stripIgnoredCharacters(sdl)).toBe(
  ...
  it('strips kitchen sink query but maintains the exact same AST', () => {
    ...
    expect(stripIgnoredCharacters(strippedQuery)).toBe(strippedQuery);
  ...
  it('strips kitchen sink SDL but maintains the exact same AST', () => {
    ...
    expect(stripIgnoredCharacters(strippedSDL)).toBe(strippedSDL);
