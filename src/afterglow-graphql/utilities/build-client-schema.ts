 ...
 * returns a GraphQLSchema instance which can be used by Afterglow execution,
...
export function buildClientSchema(
  ...
  return Effect.try({
    try: () => buildClientSchemaImpl(introspection, options),
    catch: (e) =>
      isGraphQLError(e)
        ? e
        : new GraphQLSchemaConstructionError(String((e as Error)?.message ?? e), {
            originalError: e instanceof Error ? e : undefined,
            phase: 'buildClientSchema',
          }),
  });
}

export function buildClientSchemaSync(
  introspection: IntrospectionQuery,
  options?: GraphQLSchemaValidationOptions,
): GraphQLSchema {
  return Effect.runSync(buildClientSchema(introspection, options));
}

function buildClientSchemaImpl(
  introspection: IntrospectionQuery,
  options?: GraphQLSchemaValidationOptions,
): GraphQLSchema {
  devAssert(
