...
import { executeSync } from "./execute-corpus-harness.ts";
import { getVariableValues } from "../values.ts";

const TestFaultyScalarGraphQLError = new GraphQLScalarCoercionError(
...
const TestComplexScalar = new GraphQLScalarType({
  name: 'ComplexScalar',
  parseValue(value) {
    expect(value).toBe('SerializedValue');
...
describe('Execute: Handles inputs', () => {
  ...
  describe('Handles non-nullable scalars', () => {
    ...
    it('reports error for array passed into string input', () => {
      ...
      expect(result.errors?.[0]?.originalError).toBeDefined();
    });
