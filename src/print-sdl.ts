 ...
 * each `GraphQLFieldConfig.astNode`. graphql-js's stock `printSchema` ignores
 * `astNode` entirely (see `printSchema.js` `printFields` — it concatenates
 ...
 *
 * Strategy: print via graphql-js, parse into a `DocumentNode`, walk the
