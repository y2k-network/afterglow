# @y2k-network/afterglow v1 Design Document

> **Historical design document.** This describes the original v1 design,
> including pieces (HTTP/WebSocket transports, executors) that live on the
> `feat/afterglow` branch rather than in this package. The current public
> API is the Layer-driven design in [V2_DESIGN.md](./V2_DESIGN.md); this
> file is kept for the rationale sections that still apply (nullability
> model, Relay contract, scalar design).
...
import { parse } from "../src/afterglow-graphql/language/parser.ts"
import type { GraphQLSchema } from "../src/afterglow-graphql/type/schema.ts"
import { validate } from "../src/afterglow-graphql/validation/validate.ts"
