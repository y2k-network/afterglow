# Contributing to @y2k-network/afterglow
...
- **Bug reports** — include a minimal reproduction (the smaller the better),
  the version of `effect`, `bun`, and `typescript` you are using,
...
the `.fuzz.test.ts` suffix; conformance tests and the relay-compiler
acceptance test live under `test/`.
...
3. If you change the public surface (`src/index.ts`), regenerate the API
   report: `bun run api:update`. Commit the updated `etc/afterglow.api.md`.
